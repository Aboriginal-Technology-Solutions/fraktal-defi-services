// make sure to test your own strategies, do not use this version in production
require('dotenv').config();

const { log } = require('console')
const { from } = require('rxjs')
const { map } = require('rxjs/operators')

const {
  ETH,
  BNB,
  CAKE,
  BBOO,
  BUSDC,
  BUSD,
  LINK,
  ADA,
  DOT,
  WBNB,
  vUSDC,
  vUSDT,
  BETH,
  vBTC,
  BUSDT,
  BTCB
} = require('./includes/tokens')

const privateKey = process.env.PRIVATE_KEY;
// your contract address
const flashLoanerAddress = process.env.FLASH_LOANER;

const { ethers } = require('ethers');
const network = `BSC`

const providerUrl = process.env[`${network}_PROVIDER`]
const signerAddress = process.env.SIGNER_ADDRESS

// uni/sushiswap ABIs
// const UniswapV2Pair = require('./abis/IUniswapV2Pair.json');
// const UniswapV2Factory = require('./abis/IUniswapV2Factory.json');

// use your own Infura node in production
const provider = new ethers.providers.JsonRpcProvider(providerUrl);

const wallet = new ethers.Wallet(privateKey, provider);

const ETH_TRADE = 10;
const DAI_TRADE = 3500;

const contracts = {
  pancakeFactory: {
    abi: require('./includes/abis/IUniswapV2Factory').abi,
    address: `0xBCfCcbde45cE874adCB698cC183deBcF17952812`,
    contract: null
  },
  bakeryFactory: {
    abi: require('./includes/abis/IUniswapV2Factory').abi,
    address: `0x01bF7C66c6BD861915CdaaE475042d3c4BaE16A7`,
    contract: null
  },
}
const pairs = [
  [WBNB, BETH],
  [WBNB, CAKE],
  [WBNB, BTCB],
  [WBNB, BUSDT],
  [WBNB, vUSDT],
  [WBNB, DOT],
  [WBNB, ADA],
  [WBNB, BUSD],
  [WBNB, BBOO],
  [WBNB, LINK],
  
]
const watchPairs = []
const getPairAddress = async (factory, pair) => {
  // log(Object.keys(factory))
  const [token0, token1] = pair
  // log({token0, token1, pair})
  const pairAddress = await factory.getPair(token0, token1)
  return pairAddress
}
const loadContract = async (name, address, abi, wallet) => {
  return contracts[name] = {
    name, address, abi, contract: new ethers.Contract(address, abi, wallet)
  }
}
const loadContracts = async (contracts, wallet) => {
  Object.keys(contracts)
    .forEach(name => {
      const {address, abi} = contracts[name]
      loadContract(name, address, abi, wallet)
    })
}
const loadPairs = async (contracts, pairs) => {
  // sushiEthDai = new ethers.Contract(
  //   await sushiFactory.getPair(wethAddress, daiAddress),
  //   UniswapV2Pair.abi, wallet,
  // );
  // uniswapEthDai = new ethers.Contract(
  //   await uniswapFactory.getPair(wethAddress, daiAddress),
  //   UniswapV2Pair.abi, wallet,
  // );
  _tmpArr = []
  // await Object.keys(contracts)
  //   .forEach(async contract => {

  //     await pairs.forEach(async (pair, index) => {
  //       // log({pair})
  //       // log(contracts[contract].name, pair[0].address, pair[1].address)
  //       const pairAddress = await getPairAddress(contracts[contract].contract, [pair[0].address, pair[1].address])
  //       if (pairAddress == `0x0000000000000000000000000000000000000000`) return 
  //       log(pairAddress)
  //       const pairContract = await new ethers.Contract(
  //         pairAddress, require('./includes/abis/IUniswapV2Pair').abi, wallet
  //       )
  //       let _tmp = {
  //         contract: contracts[contract].name,
  //         pair,
  //         pairAddress,
  //         pairContract
  //       }
  //       await _tmpArr.push( _tmp )
  //     })
  //   })


  // from(Object.keys(contracts))
  // .pipe(
  //   map(name => (contracts[name]),
  //   switchMap(exchange => from(exchange.contract.getPair()))

  // )
  //   .subscribe(name => {
  //     log({name})
  //   })
  from(pairs)
    .subscribe(x => {
      // _tmpArr.push(x)
      log(x)
    }, null, () => log(_tmpArr))

  // return (await Promise.all(_tmpArr))
};

const init = async () => {
  log(`Initializing Bot...`)
  await loadContracts(contracts, wallet)
  await loadPairs(contracts, pairs)
  // log({contracts})
}
const runBot = async () => {
  await init()
  log(`Starting Bot`)
  // const sushiFactory = new ethers.Contract(
  //   '0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac',
  //   UniswapV2Factory.abi, wallet,
  // );
  // const uniswapFactory = new ethers.Contract(
  //   '0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f',
  //   UniswapV2Factory.abi, wallet,
  // );
  // const daiAddress = '0x6b175474e89094c44da98b954eedeac495271d0f';
  // const wethAddress = '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2';

  // let sushiEthDai;
  // let uniswapEthDai;

  // const loadPairs = async () => {
  //   sushiEthDai = new ethers.Contract(
  //     await sushiFactory.getPair(wethAddress, daiAddress),
  //     UniswapV2Pair.abi, wallet,
  //   );
  //   uniswapEthDai = new ethers.Contract(
  //     await uniswapFactory.getPair(wethAddress, daiAddress),
  //     UniswapV2Pair.abi, wallet,
  //   );
  // };

  // await loadPairs();
  /**
  provider.on('block', async (blockNumber) => {
    try {
      console.log(blockNumber);

      const sushiReserves = await sushiEthDai.getReserves();
      const uniswapReserves = await uniswapEthDai.getReserves();

      const reserve0Sushi = Number(ethers.utils.formatUnits(sushiReserves[0], 18));

      const reserve1Sushi = Number(ethers.utils.formatUnits(sushiReserves[1], 18));

      const reserve0Uni = Number(ethers.utils.formatUnits(uniswapReserves[0], 18));
      const reserve1Uni = Number(ethers.utils.formatUnits(uniswapReserves[1], 18));

      const priceUniswap = reserve0Uni / reserve1Uni;
      const priceSushiswap = reserve0Sushi / reserve1Sushi;

      const shouldStartEth = priceUniswap < priceSushiswap;
      const spread = Math.abs((priceSushiswap / priceUniswap - 1) * 100) - 0.6;

      const shouldTrade = spread > (
        (shouldStartEth ? ETH_TRADE : DAI_TRADE)
         / Number(
           ethers.utils.formatEther(uniswapReserves[shouldStartEth ? 1 : 0]),
         ));

      console.log(`UNISWAP PRICE ${priceUniswap}`);
      console.log(`SUSHISWAP PRICE ${priceSushiswap}`);
      console.log(`PROFITABLE? ${shouldTrade}`);
      console.log(`CURRENT SPREAD: ${(priceSushiswap / priceUniswap - 1) * 100}%`);
      console.log(`ABSLUTE SPREAD: ${spread}`);

      if (!shouldTrade) return;

      const gasLimit = await sushiEthDai.estimateGas.swap(
        !shouldStartEth ? DAI_TRADE : 0,
        shouldStartEth ? ETH_TRADE : 0,
        flashLoanerAddress,
        ethers.utils.toUtf8Bytes('1'),
      );

      const gasPrice = await wallet.getGasPrice();

      const gasCost = Number(ethers.utils.formatEther(gasPrice.mul(gasLimit)));

      const shouldSendTx = shouldStartEth
        ? (gasCost / ETH_TRADE) < spread
        : (gasCost / (DAI_TRADE / priceUniswap)) < spread;

      // don't trade if gasCost is higher than the spread
      if (!shouldSendTx) return;

      const options = {
        gasPrice,
        gasLimit,
      };
      const tx = await sushiEthDai.swap(
        !shouldStartEth ? DAI_TRADE : 0,
        shouldStartEth ? ETH_TRADE : 0,
        flashLoanerAddress,
        ethers.utils.toUtf8Bytes('1'), options,
      );

      console.log('ARBITRAGE EXECUTED! PENDING TX TO BE MINED');
      console.log(tx);

      await tx.wait();

      console.log('SUCCESS! TX MINED');
    } catch (err) {
      console.error(err);
    }
  });
  */
};

console.log('Fraktal Price Bot...');

runBot();